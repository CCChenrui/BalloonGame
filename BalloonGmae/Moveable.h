...
public:
	...
	Moveable(sizeofScreenBuffer myScreenBufferSize, location myLoc, SPEED spd, DIRECTION dir, int pointsIfMoveableWins=1, int pointsIfCosmoWins=1,BALLOON_STATE type=IS_BEE);
	void setDirection(DIRECTION dir);
	void setSpeed(SPEED spd); 

	inline int getX(){return myLoc.x;};
	inline int getY(){return myLoc.y;};
//	inline BALLOON_STATE getType() {return type;};
